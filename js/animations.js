...
(function () {
  ...
  window.playHeroIntro = function () {
    ...
    tl.set('.hero-word .glyph', { yPercent: 120, opacity: 0 })
      .set('.hero-eyebrow, .hero-tags, .hero-cta-row, .hero-signature, .hero-based, .scroll-cue, .hero-console, .hero-orbit', { autoAlpha: 0, y: 16 })
      ...
      .to('.hero-tags, .hero-cta-row, .hero-signature, .hero-based, .scroll-cue, .hero-console, .hero-orbit', { autoAlpha: 1, y: 0, duration: 0.7, stagger: 0.08 }, '-=0.4');
  };
  ...
  gsap.utils.toArray('.project-card').forEach((el) => {
    const bits = el.querySelectorAll('.project-card-cap, .project-card-desc, .project-watermark, .project-card-tags, .project-card-badge, .project-card-meta');
  ...
  /* ---------- Milestones / services / why ---------- */
  ['.milestone', '.service-card', '.why-row'].forEach((sel) => {
  ...
  /* ---------- Build journey + playground ---------- */
  gsap.utils.toArray('.journey-panel').forEach((el, i) => {
    gsap.from(el, {
      autoAlpha: 0, y: 28, duration: 0.75, delay: i * 0.05, ease: 'power2.out',
      scrollTrigger: { trigger: '.journey', start: 'top 82%' }
    });
  });
  gsap.from('.playground-shell', {
    autoAlpha: 0, y: 28, duration: 0.8, ease: 'power2.out',
    scrollTrigger: { trigger: '.playground', start: 'top 82%' }
  });
